...
labels = list(set(train["Sub Category"].tolist()))
...
train["labels"] = train["Sub Category"].map(lambda x: label2id[x.strip()])
print(f"train data label encoded...\n -shape: {train.shape}")

# test data label encoding
test["labels"] = test["Sub Category"].map(lambda x: label2id[x.strip()])
